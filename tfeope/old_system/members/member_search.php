...
// Web paths
$WEB_MEMBER = rtrim(dirname($_SERVER['SCRIPT_NAME']), '/\\');              
$WEB_ROOT   = rtrim(dirname(dirname($_SERVER['SCRIPT_NAME'])), '/\\');     

if ($WEB_ROOT === '/' || $WEB_ROOT === '\\') $WEB_ROOT = '';
$WEB_ROOT = ltrim($WEB_ROOT, '\\'); // safety
...
$templateUrl = find_asset('id_template', $exts, $FS_MEMBER, $WEB_MEMBER);
$stampUrl    = find_asset('Certified', $exts, $FS_MEMBER, $WEB_MEMBER)
            ?: find_asset('certified', $exts, $FS_MEMBER, $WEB_MEMBER)
            ?: find_asset('certified2', $exts, $FS_MEMBER, $WEB_MEMBER);
...
if (!preg_match('/^TFOEPE[0-9]{7,8}$/', $search_id)) {
  return "
  ...
  try {
    ...
    if ($res && $res->num_rows === 1) {
      ...
      if ($pic !== "") {
        ...
        $uploadFsPath = $fsRoot . DIRECTORY_SEPARATOR . "uploads" . DIRECTORY_SEPARATOR . $file; // ROOT uploads
        if (file_exists($uploadFsPath)) {
          $webRoot = rtrim($webRoot, '/');
          $photoUrl = ($webRoot === "" ? "" : $webRoot) . "/uploads/" . $file;
        }
      }
...
" . ($active && $stampUrl ? "<img src='" . htmlspecialchars($stampUrl) . "' class='id-stamp-img' alt='Certified Stamp'>" : "") . "
      ";
    }
...
<div class='verify-modal-sub'><i>No record matched this Membership ID.</i></div>
...
if (isset($_GET['id'])) {
  $showModal = true;
  $modalHTML = build_modal($conn, (string)$_GET['id'], $templateUrl, $stampUrl, $WEB_ROOT, $FS_ROOT);
}
?>
<!DOCTYPE html>
<html lang="en">
...
<div class="verify-modal" id="verifyModal">
  <div class="verify-modal-card" role="dialog" aria-modal="true">
    <button class="verify-close" type="button" onclick="window.location='/'">
...
<script>
// Preload background image (inside /member)
...
// Remove blur once member photo is loaded
