...
export function AppointedPage({ items = [], query = '' }) {
  ...
  const committeeFilteredItems = selectedCommittee === 'all'
    ? regionFilteredItems
  ...
  const pageSize = 10
  const totalPages = Math.max(1, Math.ceil(committeeFilteredItems.length / pageSize))
  const pageStart = (currentPage - 1) * pageSize
  const visibleItems = committeeFilteredItems.slice(pageStart, pageStart + pageSize)
  const displayStart = committeeFilteredItems.length ? pageStart + 1 : 0
  const displayEnd = committeeFilteredItems.length ? Math.min(pageStart + pageSize, committeeFilteredItems.length) : 0
          ...
          <>
            <div className="members-toolbar members-toolbar--inline">
              <div className="members-toolbar__line">
                ...
                <p className="members-toolbar__info">{committeeFilteredItems.length} record(s)</p>
              </div>
            </div>

            {!committeeFilteredItems.length ? (
              <div className="content-empty-state">
              ...
              <div className="appointed-table-wrap">
                    ...
                    {visibleItems.map((item, index) => {
                      ...
                      return (
                        <tr key={item?.id || `appointed-${pageStart + index}`}>
                          ...
                          <td data-label="Committee / Club">{club}</td>
                          <td data-label="Region">{region}</td>
                          <td data-label="Updated">{formatDate(item?.updatedAt || item?.updated_at || item?.createdAt || item?.created_at)}</td>
                        </tr>
                      )
                ...
                <div className="table-pagination">
                  <div className="table-pagination__slot table-pagination__slot--left">
                    <p className="table-pagination__info">
                      Showing {displayStart}-{displayEnd} of {committeeFilteredItems.length}
                    </p>
                  </div>
                  <div className="table-pagination__slot table-pagination__slot--center">
                    <p className="table-pagination__info">10 rows per page</p>
